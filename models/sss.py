...
class SimpleSkinModel2(nn.Module):
    ...
    def execute(self, vertices: jt.Var, real_joints: jt.Var):
        # (B, latents)


        x = self.pct(vertices.permute(0, 2, 1))
        joints = self.mlp(x)
        shape_latent = self.relu(x)
        # (B, N, latents)
        vertices_latent = (
        ...
        if real_joints is not None:
            joints_latent = (
                self.joint_mlp(concat([real_joints.reshape(real_joints.shape[0], self.num_joints, 3),
                                       shape_latent.unsqueeze(1).repeat(1, self.num_joints, 1)], dim=-1))
            )
        else:
            joints_latent = (
                self.joint_mlp(concat([joints.reshape(joints.shape[0], self.num_joints, 3),
                                       shape_latent.unsqueeze(1).repeat(1, self.num_joints, 1)], dim=-1))
            )
        # (B, N, num_joints)
...
def create_model(model_name='pct', feat_dim=256, **kwargs):
    if model_name == "pct":
        return SimpleSkinModel2(feat_dim=feat_dim, num_joints=52)
