...
class SimpleSkinModel(nn.Module):
    ...
    def execute(self, vertices: jt.Var):
        ...
        joints_latent = (
            self.joint_mlp(concat(
                [joints.reshape(joints.shape[0], 22, 3), shape_latent.unsqueeze(1).repeat(1, self.num_joints, 1)],
                dim=-1))
        )
...
class SimpleSkinModel2(nn.Module):
    ...
    def execute(self, vertices: jt.Var):
        ...
        # (B, num_joints, latents)
        joints_latent = (
            self.joint_mlp(concat([joints.reshape(joints.shape[0], 22, 3), shape_latent.unsqueeze(1).repeat(1, self.num_joints, 1)], dim=-1))
        )
