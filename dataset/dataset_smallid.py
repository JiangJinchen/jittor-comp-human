    ...
    def __getitem__(self, index) -> Dict:
        ...
        if self.random_pose:
            ...
            if self.train:
                ...
                # trans_len = len([0 for i in os.listdir(os.path.join(self.data_root, "train")) if
                #                  i.__contains__(self.data_name) and len(i.split("-")) > 1 and int(
                #                      i.split("-")[-1]) > 0])
                # print(trans_len)
                transform_id = 0 + int(index / total_paths * 1512) + 1
                # print(transform_id)
                # transform_id = 1000 + int(index / total_paths * 512) + 1
                if 'vroid/' in path:
    ...
    def collate_batch(self, batch):
        if self.return_origin_vertices:
            ...
            for b in batch:
                ...
                b['origin_vertices'] = np.pad(b['origin_vertices'], ((0, max_N - N), (0, 0)), 'constant',
                                              constant_values=0.)
                b['N'] = N
...
def get_dataloader(
    ...
    """
    # with open("data1/r-root!x_l69.txt", 'r') as f:
    with open(data_list, 'r') as f:
        paths = [line.strip() for line in f.readlines() if line.__contains__(data_name) and int(line.split("/")[-1].split(".")[0]) < 10000]
    print("len", len(paths))
    dataset = RigDataset(
        data_root=data_root,
        paths=paths,
