...
def predict(args):
    ...
    for batch_idx, data in enumerate(predict_loader):
        ...
        for i in range(outputs.shape[0]):
            t = J2J(outputs[i].reshape(-1, 3), joints[i].reshape(-1, 3)).item()
            # print("J2J loss:", t)
            J2J_loss += t
...
def main():
    output = 'skeleton_track_1024_v_1e-5_12000+test+track'
    ...
    parser.add_argument('--pretrained_model', type=str, default='checkpoint/skeleton_v_larger_id/best_model_440.pkl',
                        help='Path to pretrained model')
    parser.add_argument('--batch_size', type=int, default=1,
                        help='Batch size for training')
    parser.add_argument('--num_ver', type=int, default=ver_num,
                        help='Batch size for training')
